...
import { getNews as getAVNews } from '../services/alphaVantage.js';
...
newsRoutes.get('/news', async (req, res) => {
  try {
    const news = await getAVNews();
    if (news.length === 0) throw new Error('No news returned');
    res.json(news);
  } catch (err) {
    console.warn('Alpha Vantage news failed, using fallback:', err.message);
